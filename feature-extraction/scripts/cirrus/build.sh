...
FE_ROOT_DIR=$CIRRUS_SCRIPT_PATH/../..
BUILD_PATH=$CIRRUS_SCRIPT_PATH/build/$backend
...
sbatch --exclusive --nodes=1 --time=24:00:00 --partition=$queue \
  --qos=$qos $ngpus --account=$ACCOUNT --job-name=fe-$queue-build \
  --output=$BUILD_PATH/$queue-report.out \
  --error=$BUILD_PATH/$queue-report.err \
  $CIRRUS_SCRIPT_PATH/build.fe.slurm $CIRRUS_SCRIPT_PATH $FE_ROOT_DIR $BUILD_PATH $backend $morpheus_dir $kokkos_dir