...
DATA_PATH=$ARCHER2_SCRIPT_PATH/../../../data/$DATA_SET/matrices
RUN_PATH=$ARCHER2_SCRIPT_PATH/run/$DATA_SET/$backend
EXE=$ARCHER2_SCRIPT_PATH/build/$backend/src/SparseTree_SpMV
...
while [ $LOW_BOUND -lt $MAX_BOUND ]; do
  ...
  sbatch --exclusive --nodes=1 --time=24:00:00 --partition=$queue \
    ...
    $ARCHER2_SCRIPT_PATH/run.profiling.slurm $RUN_PATH $DATADIR $EXE $backend $reps
