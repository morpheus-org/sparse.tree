...
PROFILING_ROOT_DIR=$A64FX_SCRIPT_PATH/../..
BUILD_PATH=$A64FX_SCRIPT_PATH/build/$backend
...
qsub -q $queue -l select=1:ncpus=48 -l place=excl \
     -o $BUILD_PATH/$queue-report.out -e $BUILD_PATH/$queue-report.err \
     -N "profiling-$queue-build" \
     -v SCRIPTPATH=$A64FX_SCRIPT_PATH,ROOT_DIR=$PROFILING_ROOT_DIR,BUILD_PATH=$BUILD_PATH,BACKEND=$backend,MORPHEUS_PATH=$morpheus_dir,KOKKOS_PATH=$kokkos_dir \
     $A64FX_SCRIPT_PATH/build.profiling.pbs
