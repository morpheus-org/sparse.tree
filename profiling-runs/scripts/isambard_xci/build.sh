...
PROFILING_ROOT_DIR=$XCI_SCRIPT_PATH/../..
BUILD_PATH=$XCI_SCRIPT_PATH/build/$backend
...
qsub -q $queue -l select=1:ncpus=32 -l place=excl \
     ...
     -v SCRIPTPATH=$XCI_SCRIPT_PATH,ROOT_DIR=$PROFILING_ROOT_DIR,BUILD_PATH=$BUILD_PATH,BACKEND=$backend,MORPHEUS_PATH=$morpheus_dir,KOKKOS_PATH=$kokkos_dir \
     $XCI_SCRIPT_PATH/build.profiling.pbs
