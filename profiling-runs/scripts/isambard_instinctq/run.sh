...
DATA_PATH=$INSTINCT_SCRIPT_PATH/../../../data/$DATA_SET/matrices
RUN_PATH=$INSTINCT_SCRIPT_PATH/run/$DATA_SET/$backend
EXE=$INSTINCT_SCRIPT_PATH/build/$backend/src/SparseTree_SpMV

echo -e "Building Feature Extraction routine:"
echo -e "\tScript Path : $INSTINCT_SCRIPT_PATH"
...
while [ $LOW_BOUND -lt $MAX_BOUND ]; do
  ...
  qsub -q $queue -l select=1:mem=128gb:ncpus=32 -l place=excl -l walltime=24:00:00 \
     ...
     $INSTINCT_SCRIPT_PATH/run.profiling.pbs
