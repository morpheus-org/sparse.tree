...
BASELINE_MAX_DEPTH = clf.tree_.max_depth
...
depths = []
while depth > 1:
    clf = DecisionTreeClassifier(random_state=10, max_depth=depth)
    clf = clf.fit(split["train"]["data"], split["train"]["target"])
    clf.extract(
        os.path.join(experiment_path, f"tree-{depth}.txt"),
    ...
    fmisses = os.path.join(experiment_path, f"misses-{depth}.csv")
    print(f"Writing Misses (depth={depth}): ", fmisses)
...
with open(facc, "w") as f:
    header = "experiment,depth,system,backend,dataset,set,total_set_matrices,misses,accuracy\n"
    f.write(header)
    total_set_matrices = split["val"]["target"].shape[0]
    for clf, misses, depth in zip(clfs, clf_misses, depths):
        accuracy = (1 - misses / nval_samples) * 100
        f.write(
            f"{experiment},{depth},{system},{backend},{dataset},val,{total_set_matrices},{misses},{accuracy:.4f}\n"
        )
