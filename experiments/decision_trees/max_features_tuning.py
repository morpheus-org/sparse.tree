...
experiment = "max_features_tuning"
...
NFEATURES = split["train"]["data"].shape[1]
clfs = []
clf_misses = []
max_features = []
for nfeatures in range(1, NFEATURES + 1):
    clf = DecisionTreeClassifier(random_state=10, max_features=nfeatures)
    clf = clf.fit(split["train"]["data"], split["train"]["target"])

    clf.extract(
        os.path.join(experiment_path, f"tree-{nfeatures}.txt"),
    ...
    print(f"Writing Misses (max_features={nfeatures}): ", fmisses)
    with open(fmisses, "w") as f:
        f.write(misses_str)

    clfs.append(clf)
    clf_misses.append(misses)
    max_features.append(str(nfeatures))

for nfeatures in ["sqrt", "log2"]:
    clf = DecisionTreeClassifier(random_state=10, max_features=nfeatures)
    clf = clf.fit(split["train"]["data"], split["train"]["target"])
...
with open(facc, "w") as f:
    header = "experiment,max_features,system,backend,dataset,set,total_set_matrices,misses,accuracy\n"
    f.write(header)
    total_set_matrices = split["val"]["target"].shape[0]
    for clf, misses, nfeatures in zip(clfs, clf_misses, max_features):
        accuracy = (1 - misses / nval_samples) * 100
        f.write(
            f"{experiment},{nfeatures},{system},{backend},{dataset},val,{total_set_matrices},{misses},{accuracy:.4f}\n"
        )
