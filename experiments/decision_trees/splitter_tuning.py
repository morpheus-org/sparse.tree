...
for split_method in ["best", "random"]:
    clf = DecisionTreeClassifier(random_state=10, splitter=split_method)
    clf = clf.fit(split["train"]["data"], split["train"]["target"])

    clf.extract(
        os.path.join(experiment_path, f"tree-{split_method}.txt"),
    ...
    print(f"Writing Misses (splitter={split_method}): ", fmisses)
    with open(fmisses, "w") as f:
        f.write(misses_str)

    clfs.append(clf)
    clf_misses.append(misses)
    splitters.append(split_method)

facc = os.path.join(experiment_path, "accuracy.csv")
print(f"Writing Accuracy: ", facc)
with open(facc, "w") as f:
    header = "experiment,splitter,system,backend,dataset,set,total_set_matrices,misses,accuracy\n"
    f.write(header)
    total_set_matrices = split["val"]["target"].shape[0]
    for clf, misses, split_method in zip(clfs, clf_misses, splitters):
        accuracy = (1 - misses / nval_samples) * 100
        f.write(
            f"{experiment},{split_method},{system},{backend},{dataset},val,{total_set_matrices},{misses},{accuracy:.4f}\n"
        )
