...
for min_sample in range(1, 10):
    clf = DecisionTreeClassifier(random_state=10, min_samples_leaf=min_sample)
    clf = clf.fit(split["train"]["data"], split["train"]["target"])

    clf.extract(
        os.path.join(experiment_path, f"tree-{min_sample}.txt"),
    ...
    print(f"Writing Misses (leaf_samples={min_sample}): ", fmisses)
...
with open(facc, "w") as f:
    header = "experiment,min_samples_leaf,system,backend,dataset,set,total_set_matrices,misses,accuracy\n"
    f.write(header)
    total_set_matrices = split["val"]["target"].shape[0]
    for clf, misses, min_sample in zip(clfs, clf_misses, min_samples):
        accuracy = (1 - misses / nval_samples) * 100
        f.write(
            f"{experiment},{min_sample},{system},{backend},{dataset},val,{total_set_matrices},{misses},{accuracy:.4f}\n"
        )
